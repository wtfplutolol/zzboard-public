...
CHANGELOG = [
    ("v1.9", [
        "Settings edit inline — cat name, Spotify keys, no separate screen",
        "Blinking cursor while typing in settings, Enter to save, Esc to cancel",
        "Settings changes apply live without restart",
        "Atomic config saves — .bak restored automatically if save is interrupted",
        "Only one instance allowed — second one exits cleanly",
        "First launch city setup no longer wiped by splash screen",
        "No internet on launch now times out in ~3s instead of hanging",
        "Clear Spotify cache added in Settings > Advanced",
        "City prompt text is now pink on first launch",
        "Snake infinite loop fix when board is full",
        "Socket leaks fixed in IP fetch and connectivity check",
    ]),
    ("v1.8 - Cat Edition", [
