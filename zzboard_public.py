...
# Keys: 1-5 switch tabs | S=snake | F=flappy | WASD/arrows=move | Space=flap | Q=quit game
...
def first_launch_setup():
    """Ask user for their city on first launch."""
    console.clear()
    t = Text(justify="center")
    t.append("\n\n")
    colors = [CLR_CLOCK, CLR_WEATHER, CLR_MOON, CLR_TASKS, CLR_CPU]
    logo = [
    ...
    t.append("\n")
    t.append("  Welcome! First time setup\n\n", style=f"bold {CLR_CLOCK}")
    console.print(Align.center(t))
    console.print()

    city = console.input(f"  [bold {CLR_WEATHER}]Enter your city for weather (e.g. London, New York, Tokyo):[/bold {CLR_WEATHER}] ").strip()
    if not city:
        city = "New York"

    tasks_input = console.input(f"  [bold {CLR_TASKS}]Enter your tasks separated by commas (or press Enter for defaults):[/bold {CLR_TASKS}] ").strip()
    if tasks_input:
        tasks = [t.strip() for t in tasks_input.split(",")]
    else:
        tasks = ["Review pull requests", "Update dependencies", "Write docs"]

    cfg = {"city": city, "tasks": tasks}
    save_config(cfg)
    console.print(f"\n  [bold {CLR_CLOCK}]Setup complete! Starting ZZBoard...[/bold {CLR_CLOCK}]\n")
    time.sleep(1.5)
    return cfg
...
def get_moon_phase():
    ...
    phase_pct = (diff % cycle) / cycle
    phase_idx = int(phase_pct * 8) % 8
    phases = [
        "New Moon", "Waxing Crescent", "First Quarter", "Waxing Gibbous",
        "Full Moon", "Waning Gibbous", "Last Quarter", "Waning Crescent"
    ]
    illumination = int(abs(math.sin(phase_pct * math.pi)) * 100)
    days_into    = phase_pct * 29.53
    days_to_full = 14.765 - days_into if days_into < 14.765 else 29.53 - days_into + 14.765
    return phases[phase_idx], illumination, abs(int(days_to_full))
...
def moon_panel():
    ...
    for line in art:
        t.append(f"  {line}\n", style=CLR_MOON)
    t.append(f"\n  {phase}\n",           style=f"bold {CLR_MOON}")
    t.append(f"  {illum}% illuminated\n", style=f"dim {CLR_MOON}")
    t.append(f"\n  {days}d to full moon\n", style=f"italic dim {CLR_DIM}")
    return Panel(t, title=f"[{CLR_MOON}]> MOON PHASE[/{CLR_MOON}]", border_style=CLR_MOON, box=box.ROUNDED)
...
def get_weather_url(city):
    try:
        ...
        ), r.get("name", city), r.get("country_code", "")
    except Exception:
        return None, city, ""
...
def fetch_weather(city):
    ...
    weather_meta["url"]     = url
    weather_meta["city"]    = name
    weather_meta["country"] = country
    while True:
...
def weather_panel():
    ...
    if not d:
        ...
        try:
            ...
            t.append(f"  [{icon}] ", style=f"bold {CLR_WEATHER}")
            t.append(f"{temp:.0f}F", style=f"bold {CLR_WEATHER}")
            t.append(f"   {desc}\n", style=f"dim {CLR_WEATHER}")
            t.append(f"\n  wind  {wind:.0f} mph\n",  style=f"dim {CLR_WEATHER}")
    ...
    return Panel(t, title=f"[{CLR_WEATHER}]> WEATHER[/{CLR_WEATHER}]", border_style=CLR_WEATHER, box=box.ROUNDED)
...
game_state = {
    "mode": "none",
    "snake_body": [], "snake_dir": (1, 0), "snake_food": (0, 0),
    "snake_score": 0, "snake_alive": True,
    "flappy_y": 11.0, "flappy_vel": 0.0, "flappy_pipes": [],
    "flappy_score": 0, "flappy_alive": True,
}
...
def snake_place_food():
    ...
    while True:
        ...
        if f not in body:
            game_state["snake_food"] = f
            break

def snake_tick():
    if not game_state["snake_alive"]: return
    dx, dy = game_state["snake_dir"]
    hx, hy = game_state["snake_body"][0]
    nx, ny = hx+dx, hy+dy
    ...
    if (nx,ny) == game_state["snake_food"]:
        game_state["snake_score"] += 1; snake_place_food()
    else:
...
def flappy_tick():
    ...
    game_state["flappy_vel"] += 0.35
    game_state["flappy_y"]   += game_state["flappy_vel"]
    ...
    for p in game_state["flappy_pipes"]:
        p["x"] -= 1
        if p["x"] == bx and (by < p["gap"] or by > p["gap"]+4):
            game_state["flappy_alive"] = False; return
...
def render_snake():
    ...
    if not game_state["snake_alive"]:
        t.append(f"\n  GAME OVER!  Score: {game_state['snake_score']}  Press S to restart\n", style=f"bold {CLR_SNAKE}")
    else:
        t.append(f"\n  Score: {game_state['snake_score']}   WASD / arrow keys   Q to quit\n", style=f"dim {CLR_SNAKE}")
    return t
...
def do_restart():
    subprocess.Popen([sys.executable if not IS_EXE else THIS_FILE,
                      *([] if IS_EXE else [THIS_FILE]), "--no-splash"])
    sys.exit(0)

def smooth_bar(pct, width, color):
    t = Text()
    fill = int(pct / 100 * width)
    t.append("[", style=f"dim {CLR_DIM}")
    t.append("#" * fill,           style=f"bold {color}")
    t.append("-" * (width - fill), style=f"dim {CLR_DIM}")
    t.append("]", style=f"dim {CLR_DIM}")
    return t
...
def spark_line(history, width, color):
    ...
    for v in list(history)[-width:]:
        t.append(bars[min(16, int(v/100*16))], style=color)
    return t
...
def check_for_update():
    ...
    try:
        r = requests.get(GITHUB_RAW_URL, verify=False, timeout=8)
        r.raise_for_status()
        remote_code = r.text
        update_status["log"].append("comparing versions...")
        script_path = os.path.join(THIS_DIR, "zzboard_public.py") if IS_EXE else THIS_FILE
        local_hash  = file_hash(script_path)
...
def cat_screen(extra_line="", progress=None):
    ...
    for i, line in enumerate(ZZBOARD_LOGO):
        t.append(line + "\n", style=f"bold {colors[i % len(colors)]}")
    t.append(f"{'  PUBLIC EDITION':^50}\n\n", style=f"dim {CLR_MOON}")
    cat_idx = int(time.time() * 3) % len(CAT_FRAMES)
    for line in CAT_FRAMES[cat_idx]:
        t.append(line + "\n", style=CLR_CLOCK)
    t.append("\n")
    ...
    if progress is not None:
        fill = int(progress)
        t.append("\n  [", style=f"dim {CLR_DIM}")
        t.append("#" * fill,        style=CLR_CLOCK)
        t.append("-" * (36-fill),   style=f"dim {CLR_DIM}")
        t.append("]\n", style=f"dim {CLR_DIM}")
...
def show_update_screen():
    ...
    while not done.is_set():
        ...
        with Live(cat_screen(f"> {log}"), console=console, screen=True, refresh_per_second=10):
            time.sleep(0.15)
    msg = "> updated! restarting..." if update_status["updated"] else "> up to date!" if not update_status["error"] else "> continuing offline..."
    with Live(cat_screen(msg), console=console, screen=True, refresh_per_second=10):
        time.sleep(2.0)
...
def show_splash():
    for tick in range(36):
        ...
        with Live(cat_screen(f"> {step}", tick), console=console, screen=True, refresh_per_second=10):
            time.sleep(0.1)
    time.sleep(0.3)
...
class ReloadHandler(FileSystemEventHandler):
    def on_modified(self, event):
        if os.path.abspath(event.src_path) == THIS_FILE:
            reload_flag.set()

def start_watcher():
    if IS_EXE or Observer is None: return
    h = ReloadHandler(); o = Observer()
    o.schedule(h, path=os.path.dirname(THIS_FILE) or ".", recursive=False)
    o.start()

# ── Speed test ─────────────────────────────────────────────────────────────────

def run_speed_test():
    if not speedtest_lib:
        speed_cache["error"] = "pip install speedtest-cli"; return
    try:
...
def fetch_speed():
    run_speed_test()
    while True:
        time.sleep(1800); run_speed_test()
...
def clock_panel():
    ...
    t    = Text(justify="center")
    t.append("\n")
    t.append(f"{now.strftime('%H')}{sep}{now.strftime('%M')}{sep}{now.strftime('%S')}", style=f"bold {CLR_CLOCK}")
    t.append("\n")
    t.append(now.strftime("%A, %d %B %Y").upper(), style=f"dim {CLR_WEATHER}")
    t.append("\n")
    t.append(f"uptime  {uptime_str()}", style=f"italic dim {CLR_DIM}")
    t.append("\n")
    return Panel(t, title=f"[{CLR_CLOCK}]  -- Z Z B O A R D  PUBLIC v1.0 --  [/{CLR_CLOCK}]", border_style=CLR_CLOCK, box=box.DOUBLE, padding=(0,2))

def cpu_panel():
    pct  = psutil.cpu_percent(interval=None)
    freq = psutil.cpu_freq()
    CPU_HIST.append(pct)
    col  = color_for(pct, CLR_CPU)
    t    = Text()
    t.append(f"\n  {pct:5.1f}%  {psutil.cpu_count()} cores", style=f"bold {col}")
    if freq: t.append(f"  {freq.current/1000:.2f} GHz", style=f"dim {CLR_CPU}")
    ...
    for i, c in enumerate(psutil.cpu_percent(percpu=True, interval=None)):
        filled = int(c/100*4)
        t.append("#"*filled + "."*(4-filled), style=color_for(c, CLR_CPU))
        t.append(" ")
        if (i+1) % 10 == 0: t.append("\n  ")
    t.append("\n")
    return Panel(t, title=f"[{CLR_CPU}]> CPU[/{CLR_CPU}]", border_style=CLR_CPU, box=box.ROUNDED)
...
def mem_panel():
    ...
    if sw.total > 0:
        ...
        t.append_text(smooth_bar(sw.percent, 40, color_for(sw.percent, CLR_MEM)))
    t.append("\n\n")
    return Panel(t, title=f"[{CLR_MEM}]> MEMORY[/{CLR_MEM}]", border_style=CLR_MEM, box=box.ROUNDED)
...
def disk_panel():
    ...
    for p in psutil.disk_partitions(all=False)[:4]:
        try:
            ...
            col = color_for(u.percent, CLR_DISK)
            t.append(f"  {p.mountpoint[:12].ljust(12)} ")
            t.append_text(smooth_bar(u.percent, 28, col))
            t.append(f" {u.percent:3.0f}%\n", style=col)
        except Exception: continue
    net = psutil.net_io_counters()
    t.append(f"\n  up   {net.bytes_sent/1024**2:>8.1f} MB\n", style=f"dim {CLR_DISK}")
    t.append(f"  down {net.bytes_recv/1024**2:>8.1f} MB\n",   style=CLR_DISK)
    return Panel(t, title=f"[{CLR_DISK}]> DISK & NET[/{CLR_DISK}]", border_style=CLR_DISK, box=box.ROUNDED)
...
def tasks_panel(tasks):
    ...
    return Panel(t, title=f"[{CLR_TASKS}]> TASKS[/{CLR_TASKS}]", border_style=CLR_TASKS, box=box.ROUNDED)

def procs_panel():
    procs = sorted(psutil.process_iter(["pid","name","cpu_percent","memory_percent"]),
                   key=lambda p: p.info["cpu_percent"] or 0, reverse=True)[:8]
    t = Text()
    t.append(f"\n  {'PID':>6}  {'NAME':<22}{'CPU':>6}  {'MEM':>6}\n", style=f"bold {CLR_PROCS}")
    t.append(f"  {'─'*6}  {'─'*22}{'─'*6}  {'─'*6}\n", style=f"dim {CLR_DIM}")
    for p in procs:
        cpu  = p.info["cpu_percent"] or 0.0
        mem  = p.info["memory_percent"] or 0.0
        name = (p.info["name"] or "?")[:22]
        t.append(f"  {p.info['pid']:>6}  {name:<22}{cpu:>5.1f}%  {mem:>5.1f}%\n", style=color_for(cpu, CLR_PROCS))
    t.append("\n")
    return Panel(t, title=f"[{CLR_PROCS}]> TOP PROCESSES[/{CLR_PROCS}]", border_style=CLR_PROCS, box=box.ROUNDED)
...
def speed_panel():
    ...
    if speed_cache["testing"]:
        ...
        t.append(f"  running speed test{dots}\n", style=f"dim {CLR_SPEED}")
    elif speed_cache["error"]:
        t.append(f"  {speed_cache['error']}\n", style=f"dim {CLR_DIM}")
    elif speed_cache["download"] is None:
        dots = "."*(int(time.time()*2)%4)
        t.append(f"  waiting for results{dots}\n", style=f"dim {CLR_DIM}")
    else:
        dl, ul, ping = speed_cache["download"], speed_cache["upload"], speed_cache["ping"]
        mx = max(dl, ul, 100)
        t.append(f"  down  "); t.append_text(smooth_bar(dl/mx*100, 36, CLR_SPEED))
        t.append(f"  {dl:.1f} Mbps\n", style=f"bold {CLR_SPEED}")
        t.append(f"\n  up    "); t.append_text(smooth_bar(ul/mx*100, 36, CLR_SPEED))
        t.append(f"  {ul:.1f} Mbps\n", style=f"bold {CLR_SPEED}")
        t.append(f"\n  ping  {ping:.0f} ms", style=f"dim {CLR_SPEED}")
        if speed_cache["last"]:
            t.append(f"   tested {int((time.time()-speed_cache['last'])/60)}m ago\n", style=f"italic dim {CLR_DIM}")
    return Panel(t, title=f"[{CLR_SPEED}]> SPEED TEST[/{CLR_SPEED}]", border_style=CLR_SPEED, box=box.ROUNDED)

# ── Tab bar ────────────────────────────────────────────────────────────────────

def tab_bar():
    tab  = current_tab["tab"]
    tabs = [(1,"1 SYSTEM",CLR_CPU),(2,"2 SKY",CLR_MOON),(3,"3 STORAGE",CLR_DISK),(4,"4 SPEED",CLR_SPEED),(5,"5 GAMES",CLR_SNAKE)]
    t    = Text()
    for num, label, color in tabs:
        if num == tab:
            t.append(f" [{label}] ", style=f"bold {color} reverse")
        else:
            t.append(f"  {label}  ", style=f"dim {CLR_DIM}")
        t.append(" ")
    return Panel(t, border_style=CLR_DIM, box=box.SIMPLE, padding=(0,1))

# ── Tab layouts ────────────────────────────────────────────────────────────────

def build_tab1(tasks):
    layout = Layout()
...
def build_tab3(tasks):
    ...
    layout["tasks"].update(tasks_panel(tasks))
    layout["procs"].update(procs_panel())
    return layout

def build_tab4(tasks):
    return speed_panel()

def build_tab5(tasks):
    mode = game_state["mode"]
    if mode == "snake":
        return Panel(render_snake(), title=f"[{CLR_SNAKE}]> SNAKE  score:{game_state['snake_score']}  Q=quit[/{CLR_SNAKE}]", border_style=CLR_SNAKE, box=box.ROUNDED)
    elif mode == "flappy":
        return Panel(render_flappy(), title=f"[{CLR_FLAPPY}]> FLAPPY BIRD  score:{game_state['flappy_score']}  Q=quit[/{CLR_FLAPPY}]", border_style=CLR_FLAPPY, box=box.ROUNDED)
    else:
        t = Text()
        t.append("\n\n")
        t.append("  Press S to play Snake\n\n",       style=f"bold {CLR_SNAKE}")
        t.append("     WASD or arrow keys to move\n", style=f"dim {CLR_SNAKE}")
        t.append("     Eat [o] to grow\n",            style=f"dim {CLR_SNAKE}")
        t.append("     Avoid walls and yourself\n\n", style=f"dim {CLR_SNAKE}")
        t.append("  Press F to play Flappy Bird\n\n", style=f"bold {CLR_FLAPPY}")
        t.append("     SPACE to flap\n",              style=f"dim {CLR_FLAPPY}")
        t.append("     Avoid the pipes [|]\n",        style=f"dim {CLR_FLAPPY}")
        t.append("     How far can you go?\n\n",      style=f"dim {CLR_FLAPPY}")
        t.append("  Q to quit back to dashboard\n",   style=f"dim {CLR_DIM}")
        return Panel(t, title=f"[{CLR_SNAKE}]> MINI GAMES[/{CLR_SNAKE}]", border_style=CLR_SNAKE, box=box.ROUNDED)

def build_layout(tasks):
    layout = Layout()
    layout.split_column(Layout(name="tabs", size=3), Layout(name="content", ratio=1))
    layout["tabs"].update(tab_bar())
    builders = {1: build_tab1, 2: build_tab2, 3: build_tab3, 4: build_tab4, 5: build_tab5}
    layout["content"].update(builders[current_tab["tab"]](tasks))
    return layout

# ── Entry ──────────────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="ZZBoard Public v1.0")
    ...
    tasks = cfg.get("tasks", ["Review pull requests", "Update dependencies", "Write docs"])
    city  = cfg.get("city", "New York")

    # Start background threads
    threading.Thread(target=lambda: fetch_weather(city), daemon=True).start()
    ...
    try:
        with Live(build_layout(tasks), console=console, refresh_per_second=4, screen=True) as live:
            while True:
    ...
    console.print(f"\n[bold {CLR_CLOCK}]  ZZBOARD PUBLIC[/bold {CLR_CLOCK}] [dim {CLR_DIM}]terminated. sweet dreams.[/dim {CLR_DIM}]\n")
